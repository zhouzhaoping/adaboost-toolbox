...
class AdaBoostMR:
    ...
    def fit(self, X, y):
        ...
        for i in range(self.__max_iter):
            ...
            for ii in range(self.__class_num):
                bestArgs, error, classEst = buildStump(X, y[:, ii].flatten().tolist(), np.apply_over_axes(sum, D, [1,
                                                                                                                   2]).ravel())  # 获得单层决策树的参数，分类误差率，该分类器的分类结果
                ...
                for class_l0 in range(self.__class_num):
                    for class_l1 in range(self.__class_num):
                        alpha = alpha + D[dat_i, class_l0, class_l1] * (
                            self.single_weak_classifier(X[dat_i], i, class_l1) - self.single_weak_classifier(X[dat_i],
                                                                                                             i,
                                                                                                             class_l0))
            ...
            for (ii, ii_value) in enumerate(D):
                for (l0, l0_value) in enumerate(ii_value):
                    for (l1, l1_value) in enumerate(l0_value):
                        D[i + 1, l0, l1] = D[i, l0, l1] * exp(0.5 * alpha * (
                        self.single_weak_classifier(X[ii], i, l0) - self.single_weak_classifier(X[ii], i, l1)))
    ...
    def single_weak_classifier(self, X, classifier_id, label_id):
        ...
        return stumpClassify(dataMatrix, self.__weakClassArr[classifier_id][label_id]['dim'],
                             self.__weakClassArr[classifier_id][label_id]['thresh'],
                             self.__weakClassArr[classifier_id][label_id]['ineq'])
    ...
    def predict(self, X, class_ind):
        ...
        for i in range(len(self.__weakClassArr)):
            classEst = stumpClassify(dataMatrix, self.__weakClassArr[i][class_ind]['dim'],
                                     self.__weakClassArr[i][class_ind]['thresh'],
                                     self.__weakClassArr[i][class_ind]['ineq'])
